...
module.exports = {
    createUser: async (req, res) => {
        const newUser = new User({
            ...
            password: CrytoJs.AES.encrypt(req.body.password, process.env.SECRET).toString(),
        });
    ...
    loginUser: async (req, res) => {
        try {
            ...
            !user && res.status(401).json("Wrong a valid email");

            const decryptedPassword = CrytoJs.AES.decrypt(user.password, process.env.SECRET);
            const decryptedPass = decryptedPassword.toString(CrytoJs.enc.Utf8);
